...
class UserImage():
    ...
    def url_image(self):
        if self.image:
            reader = Serialize(
                app.config['SECRET_KEY_USERS'], int(timedelta(7).total_seconds())
            )
            autorization = reader.dumps(
                {'id_user': self.id_user}
            )
            self._url_image = url_for(
                "restadminimageuser",
                autorization=autorization,
                _external=True)
        else:
            self._url_image = url_for('static', filename="images/user.png")
    ...
    def set_image(self, file, filename, extensao):
        ...
        q_image = self.db(
            (db.auth_user_phantergallery.auth_user == self.id_user) &
            (db.auth_user_phantergallery.subfolder == 'profile')).select()
        if q_image:
            for q in q_image:
                try:
            ...
            db.commit()
        id_new_image = self.db.phantergallery.insert(
...
class PhanterGalleryUpload(object):
    ...
    def insert_or_update(self, file, folder, filename, extensao):
        ...
        if q_image:
            ...
            id_image = self.db.phantergallery.insert(
                folder=folder,
                filename=filename,
                extensao=extensao)
            if id_image:
